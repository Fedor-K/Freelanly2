/**
 * ============================================================================
 * SMARTRECRUITERS PROCESSOR
 * ============================================================================
 *
 * Imports jobs from SmartRecruiters ATS.
 * API: https://api.smartrecruiters.com/v1/companies/{companySlug}/postings
 *
 * Response format: { content: [...] }
 * Each job has: id, uuid, name, company, location, releasedDate,
 *   department, experienceLevel, ref
 *
 * Note: The public postings API does not include job description.
 * Description is fetched per-job from the detail endpoint.
 *
 * ============================================================================
 */
...
export async function processSmartRecruitersSource(context: ProcessorContext): Promise<ProcessingStats> {
  ...
  if (!dataSource || dataSource.sourceType !== 'SMARTRECRUITERS') {
    throw new Error('Invalid SmartRecruiters data source');
  }

  if (!dataSource.companySlug) {
    throw new Error('Company slug is required for SmartRecruiters source');
  }

  if (isBlockedCompany(dataSource.companySlug, dataSource.name)) {
    console.log(`[SmartRecruiters] BLOCKED: ${dataSource.name} is in company blacklist`);
    return stats;
  }

  const apiUrl = dataSource.apiUrl ||
    `https://api.smartrecruiters.com/v1/companies/${dataSource.companySlug}/postings`;

  try {
    console.log(`[SmartRecruiters] Fetching jobs from: ${apiUrl}`);
    ...
    const data: SmartRecruitersApiResponse = await response.json();
    const jobs = data.content || [];
    stats.total = jobs.length;
    console.log(`[SmartRecruiters] Found ${jobs.length} jobs for ${dataSource.name}`);
    ...
    const jobUrls = jobs.map(j => `https://jobs.smartrecruiters.com/${dataSource.companySlug}/${j.id}`);
    const existingJobs = await prisma.job.findMany({
      where: {
        OR: [
          { sourceId: { in: jobs.map(j => j.id) } },
          { sourceUrl: { in: jobUrls } },
        ],
      },
    ...
    // Filter jobs
    const filteredJobs: SmartRecruitersJob[] = [];
    const maxAgeDate = getMaxJobAgeDate();

    for (const job of jobs) {
      const jobUrl = `https://jobs.smartrecruiters.com/${dataSource.companySlug}/${job.id}`;
      if (existingSourceIds.has(job.id) || existingSourceUrls.has(jobUrl)) {
        stats.skipped++;
        continue;
      }

      if (job.releasedDate) {
        const jobDate = new Date(job.releasedDate);
      ...
      const filterResult = shouldSkipJob({
        ...
        locationType: detectLocationType(locationStr, job.location?.remote),
      });
    ...
    if (jobsToProcess.length > MAX_JOBS_PER_SOURCE) {
      ...
      console.log(`[SmartRecruiters] Limiting ${dataSource.name} from ${jobsToProcess.length} to ${MAX_JOBS_PER_SOURCE} jobs`);
      jobsToProcess = jobsToProcess.slice(0, MAX_JOBS_PER_SOURCE);
      stats.skipped += excess;
    }

    if (jobsToProcess.length === 0) {
      console.log(`[SmartRecruiters] No new jobs to process for ${dataSource.name}`);
      await prisma.dataSource.update({
        where: { id: dataSourceId },
        data: {
          lastRunAt: new Date(),
    ...
    console.log(`[SmartRecruiters] Processing ${jobsToProcess.length} jobs (${stats.skipped} filtered)`);
    ...
    const existingTitles = await prisma.job.findMany({
      where: {
        ...
        title: { in: jobsToProcess.map(j => j.name) }
      },
      select: { title: true }
    });
    const existingTitleSet = new Set(existingTitles.map(j => j.title));
    const jobsAfterTitleCheck = jobsToProcess.filter(j => !existingTitleSet.has(j.name));
    const titleDuplicates = jobsToProcess.length - jobsAfterTitleCheck.length;
    if (titleDuplicates > 0) {
      console.log(`[SmartRecruiters] Filtered ${titleDuplicates} duplicate titles`);
    ...
    const processJob = async (job: SmartRecruitersJob) => {
      try {
        const result = await processSmartRecruitersJob(
          job,
          company.id,
          companySlug,
          importLogId,
        ...
        console.log(`[SmartRecruiters] ${processed}/${totalToProcess} | +${created} | ETA: ${eta}s | ${job.name.slice(0, 40)}...`);
      } catch (error) {
        stats.failed++;
    ...
    if (stats.createdJobUrls && stats.createdJobUrls.length > 0) {
      console.log(`[SmartRecruiters] Notifying search engines for ${stats.createdJobUrls.length} new jobs`);
      notifySearchEngines(stats.createdJobUrls).catch((err) => {
        console.error('[SmartRecruiters] Search engine notification failed:', err);
      });
    }
...
async function fetchJobDescription(companySlug: string, jobId: string): Promise<string | null> {
  try {
    ...
    // The detail endpoint returns jobAd.sections.jobDescription.text (HTML)
    const html = data?.jobAd?.sections?.jobDescription?.text
...
async function processSmartRecruitersJob(
  ...
  const description = await fetchJobDescription(companySlug, job.id);

  if (!description || description.length < 50) {
    console.log(`[SmartRecruiters] Skipping job with insufficient content: ${job.name}`);
  ...
  const baseSlug = slugify(`${job.name}-${companySlug}-${shortId}`);
  const slug = await generateUniqueJobSlug(baseSlug);

  const locationStr = buildLocationString(job.location);
  const locationType = detectLocationType(locationStr, job.location?.remote);
  const country = job.location?.country || extractCountryCode(locationStr);
  const level = job.experienceLevel ? mapExperienceLevel(job.experienceLevel.label) : extractLevel(job.name);
  ...
  const postedAt = job.releasedDate ? new Date(job.releasedDate) : new Date();
  const jobUrl = `https://jobs.smartrecruiters.com/${companySlug}/${job.id}`;
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('Unique constraint')) {
      console.log(`[SmartRecruiters] Skipping duplicate: ${job.name}`);
      return { status: 'skipped' };
    }
    throw error;
  }
...
function buildLocationString(location?: SmartRecruitersJob['location']): string {
  if (!location) return 'Remote';
  const parts = [location.city, location.region, location.country].filter(Boolean);
  if (parts.length === 0) {
    return location.remote ? 'Remote' : 'Unknown';
  }
  return parts.join(', ');
}
...
async function findOrCreateCompany(name: string, slug: string) {
  ...
  const website = `https://${normalizedSlug.replace(/-/g, '')}.com`;
  ...
  if (!company) {
    const uniqueSlug = await generateUniqueCompanySlug(normalizedSlug);
...
function mapExperienceLevel(level: string): 'INTERN' | 'ENTRY' | 'JUNIOR' | 'MID' | 'SENIOR' | 'LEAD' | 'MANAGER' | 'DIRECTOR' | 'EXECUTIVE' {
  ...
  if (l.includes('executive') || l.includes('vp') || l.includes('chief')) return 'EXECUTIVE';
  return 'MID';
}
