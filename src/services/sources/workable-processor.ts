 ...
 * API: https://apply.workable.com/api/v1/widget/accounts/{companySlug}
 *
 * Response format: { name, jobs: [...] }
 * Each job has: title, shortcode, employment_type, telecommuting, department,
 *   url, application_url, published_on, country, city, locations[]
 *
 * Caveat: Widget API returns 404 for many companies and does NOT include
 * job description. Description must be fetched separately per shortcode.
 *
 * ============================================================================
 */
...
export async function processWorkableSource(context: ProcessorContext): Promise<ProcessingStats> {
  ...
  if (!dataSource || dataSource.sourceType !== 'WORKABLE') {
    throw new Error('Invalid Workable data source');
  }

  if (!dataSource.companySlug) {
    throw new Error('Company slug is required for Workable source');
  }

  if (isBlockedCompany(dataSource.companySlug, dataSource.name)) {
    console.log(`[Workable] BLOCKED: ${dataSource.name} is in company blacklist`);
    return stats;
  }

  const apiUrl = dataSource.apiUrl ||
    `https://apply.workable.com/api/v1/widget/accounts/${dataSource.companySlug}`;

  try {
    console.log(`[Workable] Fetching jobs from: ${apiUrl}`);

    const response = await fetch(apiUrl);
    if (!response.ok) {
      if (response.status === 404) {
        console.log(`[Workable] Company ${dataSource.companySlug} returned 404 — widget API not available`);
        await prisma.dataSource.update({
          where: { id: dataSourceId },
          data: {
            lastRunAt: new Date(),
            lastError: 'Widget API returned 404',
    ...
    console.log(`[Workable] Found ${jobs.length} jobs for ${dataSource.name}`);
    ...
    const jobUrls = jobs.map(j => j.url || `https://apply.workable.com/${dataSource.companySlug}/j/${j.shortcode}/`);
    const existingJobs = await prisma.job.findMany({
      where: {
        OR: [
          { sourceId: { in: jobs.map(j => j.shortcode) } },
    ...
    // Filter jobs
    const filteredJobs: WorkableJob[] = [];
    const maxAgeDate = getMaxJobAgeDate();

    for (const job of jobs) {
      const jobUrl = job.url || `https://apply.workable.com/${dataSource.companySlug}/j/${job.shortcode}/`;
      if (existingSourceIds.has(job.shortcode) || existingSourceUrls.has(jobUrl)) {
        stats.skipped++;
        continue;
      }

      if (job.published_on) {
        const jobDate = new Date(job.published_on);
      ...
      const filterResult = shouldSkipJob({
        ...
        locationType: detectLocationType(locationStr, job.telecommuting),
      });
    ...
    if (jobsToProcess.length > MAX_JOBS_PER_SOURCE) {
      ...
      console.log(`[Workable] Limiting ${dataSource.name} from ${jobsToProcess.length} to ${MAX_JOBS_PER_SOURCE} jobs`);
      jobsToProcess = jobsToProcess.slice(0, MAX_JOBS_PER_SOURCE);
      stats.skipped += excess;
    }

    if (jobsToProcess.length === 0) {
      console.log(`[Workable] No new jobs to process for ${dataSource.name}`);
      await prisma.dataSource.update({
        where: { id: dataSourceId },
        data: {
          lastRunAt: new Date(),
    ...
    console.log(`[Workable] Processing ${jobsToProcess.length} jobs (${stats.skipped} filtered)`);
    ...
    if (titleDuplicates > 0) {
      console.log(`[Workable] Filtered ${titleDuplicates} duplicate titles`);
    ...
    const processJob = async (job: WorkableJob) => {
      try {
        const result = await processWorkableJob(
          job,
          company.id,
          companySlug,
          importLogId,
        ...
        console.log(`[Workable] ${processed}/${totalToProcess} | +${created} | ETA: ${eta}s | ${job.title.slice(0, 40)}...`);
      } catch (error) {
        stats.failed++;
        stats.errors.push(`Job ${job.shortcode}: ${String(error)}`);
      }
    };
    ...
    if (stats.createdJobUrls && stats.createdJobUrls.length > 0) {
      console.log(`[Workable] Notifying search engines for ${stats.createdJobUrls.length} new jobs`);
      notifySearchEngines(stats.createdJobUrls).catch((err) => {
        console.error('[Workable] Search engine notification failed:', err);
      });
    }
...
async function fetchJobDescription(companySlug: string, shortcode: string): Promise<string | null> {
  try {
    // Workable job detail endpoint
    const response = await fetch(
      `https://apply.workable.com/api/v3/accounts/${companySlug}/jobs/${shortcode}`
    );
...
async function processWorkableJob(
  job: WorkableJob,
  companyId: string,
  companySlug: string,
  importLogId: string,
  ...
  const description = await fetchJobDescription(companySlug, job.shortcode);

  if (!description || description.length < 50) {
    console.log(`[Workable] Skipping job with insufficient content: ${job.title}`);
    return { status: 'skipped' };
  }

  console.log(`[Workable] Processing through AI: ${job.title}`);
  ...
  const baseSlug = slugify(`${job.title}-${companySlug}-${job.shortcode}`);
  const slug = await generateUniqueJobSlug(baseSlug);

  const locationStr = buildLocationString(job);
  const locationType = detectLocationType(locationStr, job.telecommuting);
  const country = job.country || extractCountryCode(locationStr);
  ...
  const postedAt = job.published_on ? new Date(job.published_on) : new Date();
  const jobUrl = job.url || `https://apply.workable.com/${companySlug}/j/${job.shortcode}/`;
  const applyUrl = job.application_url || jobUrl;
  const jobType = mapEmploymentType(job.employment_type);
  ...
  try {
    ...
    if (error instanceof Error && error.message.includes('Unique constraint')) {
      console.log(`[Workable] Skipping duplicate: ${job.title}`);
      return { status: 'skipped' };
    }
    throw error;
  }
...
function buildLocationString(job: WorkableJob): string {
  if (job.telecommuting) {
    if (job.city && job.country) return `Remote - ${job.city}, ${job.country}`;
...
function detectLocationType(location?: string, telecommuting?: boolean): 'REMOTE' | 'REMOTE_US' | 'REMOTE_EU' | 'REMOTE_COUNTRY' | 'HYBRID' | 'ONSITE' {
  if (telecommuting) {
