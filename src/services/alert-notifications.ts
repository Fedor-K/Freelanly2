...
// Minimum interval between cron runs (distributed lock)
const MIN_INTERVAL_MS = 60000; // 60 seconds - prevents multiple cron sources
const LOCK_KEY = 'instant_alerts_lock';

/**
 * Acquire a distributed lock using the Settings table
 * Uses optimistic locking - check timestamp before and after update
 */
async function acquireDistributedLock(): Promise<boolean> {
  const now = new Date();
  const lockExpiry = new Date(now.getTime() - MIN_INTERVAL_MS);

  try {
    // First, check if lock exists and is still valid
    const existingLock = await prisma.settings.findUnique({
      where: { key: LOCK_KEY },
    });

    if (existingLock) {
      const lockData = existingLock.value as { lockedAt?: string };
      if (lockData.lockedAt) {
        const lockedAt = new Date(lockData.lockedAt);
        if (lockedAt > lockExpiry) {
          const secondsAgo = Math.round((now.getTime() - lockedAt.getTime()) / 1000);
          console.log(`[InstantAlerts] Lock held (${secondsAgo}s ago), skipping`);
          return false;
        }
      }
    }
    ...
    // Verify we got the lock (another process might have updated it)
    const verifyLock = await prisma.settings.findUnique({
      where: { key: LOCK_KEY },
    });

    if (verifyLock) {
      const verifyData = verifyLock.value as { lockedAt?: string };
      const verifyTime = new Date(verifyData.lockedAt || 0);
      // If the lock time is within 2 seconds of our update, we got it
      if (Math.abs(verifyTime.getTime() - now.getTime()) < 2000) {
        console.log(`[InstantAlerts] Lock acquired`);
        return true;
      }
    }

    console.log(`[InstantAlerts] Lock race lost, skipping`);
    return false;
  } catch (error) {
    console.error('[InstantAlerts] Failed to acquire lock:', error);
    return false;
  }
}
...
export async function processInstantAlertQueue(): Promise<{ sent: number; failed: number; processed: number }> {
  ...
  return doProcessInstantAlertQueue();
}

async function doProcessInstantAlertQueue(): Promise<{ sent: number; failed: number; processed: number }> {
