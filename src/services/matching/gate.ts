...
export type GateInput = {
  ...
  candidateGithub?: string | null; // fresh STRONG/ACTIVE GitHub evidence line — corroborating-only (see SYSTEM)
};
export type Gate = {
...
- seniority_ok: false ONLY if job needs 5+ years and candidate is a student/intern/0-1y. NICHE-TECH EXCEPTION: if the role centers on a SPECIFIC platform/tool/framework and demands more years of it than that technology has plausibly existed (e.g. "10+ years" of a platform released ~5 years ago — Unqork, a recent low-code/AI/cloud tool, etc.), do NOT set seniority_ok=false for a candidate with MULTIPLE years of hands-on experience in that exact technology — a few years on a young niche platform is near the ceiling of available talent, so treat their niche tenure as senior-level FOR THAT NICHE.
...
- YEARS hard_fail (hard_kind="years"): set true ONLY when the post states an EXPLICIT minimum NUMBER of years (e.g. "5+ years required") AND the candidate is clearly below it. A seniority WORD alone — "Senior", "Lead", "Principal" in the title or body with no stated number — is NOT a years hard_fail (hard_fail=false). If you find yourself writing "not explicitly stated" / "likely requires more", that means hard_fail=false. NICHE-TECH EXCEPTION (same as seniority_ok): if the required years EXCEED the plausible age of the specific platform/tool the role is built around and the candidate has multiple years of hands-on experience in that exact tech, the number is an unrealistic/generic ask — hard_fail=false.
- native_language: use hard_kind="native_language" when the post requires a NATIVE / mother-tongue speaker of a language (explicit "native English speaker" / "mother-tongue German", OR a translation/interpreting/subtitling/localization role NAMED for a specific language — "Japanese Translator", "English-Korean Interpreter" — which inherently needs native-level command of that named language) AND the candidate is CLEARLY not native in it. Two strong NOT-native signals: (1) GEOGRAPHY — the named language is not plausibly the candidate's mother tongue given their LOCATION/background (e.g. a candidate located in Nigeria/India/Brazil claiming native Japanese, Korean, Dutch, or Polish). (2) INFLATION — the candidate lists MANY languages (3+), especially from unrelated regions (e.g. "Japanese, Dutch, Polish, English"), which is profile padding by an annotation/gig worker, NOT genuine native fluency in a distant language. When the required native language is geographically far from the candidate AND/OR appears in a long list of unrelated claimed languages, set hard_fail=true, hard_kind="native_language". Stay asymmetric: a candidate IN/NEAR the language's home region, or claiming just 1-2 plausible languages, is fine — hard_fail=false. (Separate from language_ok, which is only about the language PAIR.)
...
- CANDIDATE GitHub evidence (when present): independently verified public-repo evidence. CORROBORATING-ONLY — it may INCREASE your confidence that a claimed skill/profession is real (e.g. lean "adjacent" over "different" when repos back the claim). Its absence, or thin public activity, must NEVER lower any assessment — most professional work lives in private repos and most candidates provide no GitHub at all.`;
...
export async function runGate(inp: GateInput): Promise<Gate> {
  ...
  // Deterministic SENIORITY guard: the rule says seniority_ok=false should fire ONLY for a
  // student/intern/0-1y candidate. The LLM over-fires it for SENIOR-titled roles or unrealistic year
  // asks (e.g. "12 years of Unqork" — a platform far too young for that) against a candidate who
  // actually has real tenure. Seniority is the SOFTEST gap yet seniority_ok=false is a hard NO here,
  // so when the candidate clearly isn't a junior (>=2 yrs known), trust the rule, not the flag — the
  // remaining gap (if the post states an explicit year minimum) still surfaces via the years hard_fail.
  let seniorityOk = p.seniority_ok !== false;
  if (!seniorityOk && typeof inp.candidateYears === 'number' && inp.candidateYears >= 2) seniorityOk = true;
  return {
...
export function assess(
  ...
  // missing CV is a hard block (owner decision 2026-06-06): no LinkedIn fallback, no
  // cover-letter-only send. Only a genuine Blob-stored upload (resumeUrl set) passes here.
  if (!candidateHasRealCV) return { decision: 'NO', reason: 'no real CV', extras };
