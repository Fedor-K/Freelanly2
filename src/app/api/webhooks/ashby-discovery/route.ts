/**
 * Ashby Company Discovery Webhook
 *
 * Receives Google Search results from Apify actor and adds new Ashby companies to DataSource.
 */

import { NextRequest, NextResponse } from 'next/server';
import { prisma } from '@/lib/db';
import { extractUniqueSlugs } from '@/config/ashby-discovery';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Ashby Discovery] Processing ${urls.length} URLs`);

    const slugs = extractUniqueSlugs(urls);
    console.log(`[Ashby Discovery] Found ${slugs.length} unique slugs:`, slugs);

    if (slugs.length === 0) {
      return NextResponse.json({
        message: 'No valid Ashby company slugs found in URLs',
        added: 0,
        skipped: 0,
      });
    }
    ...
    const existingSources = await prisma.dataSource.findMany({
      where: { sourceType: 'ASHBY', companySlug: { in: slugs } },
    ...
    for (const slug of validSlugs) {
      try {
        ...
        console.log(`[Ashby Discovery] Added: ${slug} (${displayName})`);
      } catch (error) {
    ...
    console.log('[Ashby Discovery] Complete:', response);
...
export async function GET(request: NextRequest) {
  ...
  try {
    const ashbyCount = await prisma.dataSource.count({ where: { sourceType: 'ASHBY' } });
    const activeCount = await prisma.dataSource.count({ where: { sourceType: 'ASHBY', isActive: true } });

    const { ASHBY_SEARCH_QUERIES } = await import('@/config/ashby-discovery');

    return NextResponse.json({
      status: 'ready',
      ashbySources: { total: ashbyCount, active: activeCount },
      searchQueries: { count: ASHBY_SEARCH_QUERIES.length },
    });
  } catch (error) {
    return NextResponse.json(
