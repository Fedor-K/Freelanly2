/**
 * Cron: Discover new SmartRecruiters companies via Apify Google Search
 *
 * Runs weekly to find new companies using SmartRecruiters ATS.
 * Schedule: Once per week (e.g., Sunday ~5:30 UTC)
 */
...
import {
  extractUniqueSlugs,
  SMARTRECRUITERS_SEARCH_QUERIES,
} from '@/config/smartrecruiters-discovery';
...
export async function POST(request: NextRequest) {
  ...
  try {
    // Rotate queries (offset +3 from Lever)
    const weekNumber = Math.floor(Date.now() / (7 * 24 * 60 * 60 * 1000));
    const startIndex =
      ((weekNumber + 3) * QUERIES_PER_RUN) % SMARTRECRUITERS_SEARCH_QUERIES.length;
    const queries = [
      ...SMARTRECRUITERS_SEARCH_QUERIES.slice(startIndex, startIndex + QUERIES_PER_RUN),
      ...SMARTRECRUITERS_SEARCH_QUERIES.slice(
        0,
        Math.max(0, startIndex + QUERIES_PER_RUN - SMARTRECRUITERS_SEARCH_QUERIES.length)
      ),
    ].slice(0, QUERIES_PER_RUN);

    console.log(
      `[SmartRecruiters Discovery] Starting with ${queries.length} queries (week ${weekNumber})`
    );
    ...
    if (slugs.length === 0) {
      return NextResponse.json({
        message: 'No SmartRecruiters companies found',
    ...
    const newSlugs = slugs.filter((slug) => !existingSlugsSet.has(slug.toLowerCase()));
    ...
    // Validate new slugs against SmartRecruiters API
    ...
    for (const { slug } of validSlugs) {
      try {
        ...
        console.log(`[SmartRecruiters Discovery] Added: ${slug}`);
      } catch (error) {
    ...
    console.log('[SmartRecruiters Discovery] Complete:', result);
...
export async function GET(request: NextRequest) {
  ...
  const srCount = await prisma.dataSource.count({
    where: { sourceType: 'SMARTRECRUITERS' },
  });

  const activeCount = await prisma.dataSource.count({
    where: { sourceType: 'SMARTRECRUITERS', isActive: true },
  });

  return NextResponse.json({
    status: 'ready',
    smartrecruitersSources: { total: srCount, active: activeCount },
    queriesTotal: SMARTRECRUITERS_SEARCH_QUERIES.length,
